function [avg_mpc1,avg_mpc4,var_mpc1,var_mpc4] = simulation_MPCs(p,xsim,csim,diesim,ynetsim,yPindsim,yFindsim,...
    ...
    for im = 1:Nmpcamount
        ...
        for it = 1:4
            ...
            for ib = 1:p.nb
            for iyP = 1:p.nyP
                below_grid = xsim_mpc{im}(:,it)<xgrid.longgrid(1,iyP,iyF);
                idx = yPindsim(:,simT)==iyP & betaindsim(:,simT)==ib & yFindsim(:)==iyF;
                ...
                if mpcamount{im} < 0 && it == 1
                    ...
                    xsim_mpc{im}(idx_below,it) = xgrid.longgrid(1,iyP,iyF);
