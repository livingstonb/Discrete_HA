classdef TableGenDetailed < tables.TableGen
	properties
...
function out = wealth_constraints_table(values)
	out = tables.TableGen.new_table_with_header('WEALTH CONSTRAINTS');
...
function out = wealth_distribution_table(values)
	out = tables.TableGen.new_table_with_header('WEALTH DISTRIBUTION');
...
function out = mpcs_table(values, p, ishock, shock_size)
	...
	out = tables.TableGen.new_table_with_header(header);
...
function out = mpcs_news_table(values, p, statistic, shock_labels)
	...
	out = tables.TableGen.new_table_with_header(header_name);
...
function out = specialty_mpc_tables(values, p, shock_labels)
	...
	loss_table = tables.TableGen.new_table_with_header(header_name);
    ...
    loss_table = tables.TableGen.append_to_table(loss_table, new_entries, new_labels);

	% One-year loan
	loan_size = shock_labels{6};
	header_name = sprintf('MPCs OUT OF 4-PERIOD LOAN OF %s', loan_size);
	loan_table = tables.TableGen.new_table_with_header(header_name);
    ...
    loan_table = tables.TableGen.append_to_table(loan_table, new_entries, new_labels);
